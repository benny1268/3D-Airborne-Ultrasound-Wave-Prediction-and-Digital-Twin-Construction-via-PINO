...
class FNO2DEncoder(nn.Module):
    ...
    num_fno_layers : int, optional
        Number of spectral convolutional layers, by default 6
    ...
    num_fno_modes : Union[int, List[int]], optional
        Number of Fourier modes kept in spectral convolutions, by default 32
    padding :  Union[int, List[int]], optional
        Domain padding for spectral convolutions, by default 0
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if any(p is not None for p in self.pad):
            x = F.pad(x, (0, self.pad[1], 0, self.pad[0]), mode=self.padding_type)

        for conv, mlp in zip(self.spconv_layers_fno, self.mlp_layers_fno):
            x = mlp(conv(x)) + x

        if any(p is not None for p in self.ipad):
...
class SpectralConv2d_FFNO(nn.Module):
    ...
    def forward(self, x):
        ...
        out_ft = x_ft.new_zeros(B, self.out_channels, X, Y // 2 + 1) #torch.zeros(B, self.out_channels, X, Y//2 + 1, dtype=torch.cfloat, device=x.device)
        ...
        out_ft = x_ft.new_zeros(B, self.out_channels, X//2 + 1, Y)
...
class FNO3DEncoder(nn.Module):
    ...
    def __init__(
        ...
        if self.coord_features:
            self.in_channels += 3  # Add 3D grid coordinates

        if isinstance(padding, int):
            padding = [padding] * 3
        padding = padding + [0, 0, 0]  # ensure list length
        self.pad = padding[:3]
        self.ipad = [-pad if pad > 0 else None for pad in self.pad]
        self.padding_type = padding_type

        if isinstance(num_fno_modes, int):
            num_fno_modes = [num_fno_modes] * 3
    ...
    def forward(self, x: Tensor) -> Tensor:
        if x.dim() != 5:
            raise ValueError("Only 5D tensors [B, C, X, Y, Z] accepted for 3D FNO")
...
class SpectralConv3d_FFNO(nn.Module):
    ...
    def forward(self, x):
        ...
        out_ft = x_ft.new_zeros(B, self.out_channels, X, Y, Z // 2 + 1)
        ...
        out_ft = x_ft.new_zeros(B, self.out_channels, X, Y // 2 + 1, Z)
        ...
        out_ft = x_ft.new_zeros(B, self.out_channels, X // 2 + 1, Y, Z)
